...
function blockedProxyUrlReason(rawUrl: string, allowLocal?: boolean): string | null {
    let url: URL;
    try {
        url = new URL(rawUrl);
    } catch {
    ...
    // 调用方显式为该 MCP 服务器开启了"允许本机/内网地址"时，跳过下面的内网地址拦截。
    // 协议与 URL 合法性校验始终保留，不受此开关影响。
 ...
 *
 * POST /api/tool-proxy
 * Body: { url, method, headers, body, timeoutMs?, allowLocal? }
 * allowLocal: true 时跳过本机/内网地址拦截，仅当调用方（如某个 MCP 服务器配置）
 * 显式勾选"允许本地网络"时才应传入 true。
 */
export async function POST(req: NextRequest) {
    let requestUrlForDebug = "";
    let fetchUrlForDebug = "";
    try {
        const { url, method, headers, body, timeoutMs, allowLocal } = await req.json();
        requestUrlForDebug = typeof url === "string" ? url : "";

        if (!url || typeof url !== "string") {
            return NextResponse.json({ error: "Missing url" }, { status: 400 });
        }
        const allowLocalNetwork = allowLocal === true;
        const blockedReason = blockedProxyUrlReason(url, allowLocalNetwork);
        if (blockedReason) {
        ...
        if (method === "SSE_REQUEST") {
            try {
                ...
                const msgBlockedReason = blockedProxyUrlReason(msgUrl.toString(), allowLocalNetwork);
                if (msgBlockedReason) {
                    reader.cancel().catch(() => {});
