...
import {
  ...
  setCheckPhoneWallpaperTransform,
  setCheckPhoneIconOverride,
  setCheckPhoneLabelColor,
...
const CHECKPHONE_LABEL_COLOR_PRESETS: string[] = [
  "#777777",
  "#ffffff",
  "#111111",
  "#ff3b30",
...
export function CheckPhoneApp({ onClose }: CheckPhoneAppProps) {
  ...
  const [appearanceTab, setAppearanceTab] = useState<"wallpaper" | "icons">("wallpaper");
  ...
  function applyWallpaperTransform(patch: { wallpaperScale?: number; wallpaperOffsetX?: number; wallpaperOffsetY?: number }) {
    if (!activeCharId) return;
    const next = setCheckPhoneWallpaperTransform(activeCharId, patch);
    setAppearanceMap((prev) => ({ ...prev, [activeCharId]: next }));
  }
  ...
  function applyLabelColor(color: string | null) {
    if (!activeCharId) return;
    const next = setCheckPhoneLabelColor(activeCharId, color);
    setAppearanceMap((prev) => ({ ...prev, [activeCharId]: next }));
  }
    ...
    return (
      <div className="cp-fullscreen-simulator">
        <div
          ...
          style={activeAppearance?.labelColor ? ({ "--cp-label-color": activeAppearance.labelColor } as CSSProperties) : undefined}
        >
          {activeAppearance?.wallpaperUrl && (
            <div className="cp-wallpaper-layer" aria-hidden="true">
              <img
                src={activeAppearance.wallpaperUrl}
                alt=""
                style={{
                  transform: `translate(-50%, -50%) translate(${activeAppearance.wallpaperOffsetX}%, ${activeAppearance.wallpaperOffsetY}%) scale(${activeAppearance.wallpaperScale / 100})`,
                }}
              />
            </div>
          )}
          ...
          {!isEmbeddedAppOpen && (
            <div className="cp-floating-controls">
              <div className="cp-floating-left-cluster">
          ...
          {appearanceOpen && (
            <div
              className="cp-appearance-modal-overlay"
              role="dialog"
              aria-modal="true"
              aria-label="自定义壁纸和图标"
              onClick={() => setAppearanceOpen(false)}
            >
              <div className="cp-appearance-modal" onClick={(event) => event.stopPropagation()}>
                <div className="cp-desktop-settings-head cp-appearance-modal-head">
                  <span>自定义外观</span>
                  <button
                    type="button"
                    className="cp-appearance-modal-close"
                    aria-label="关闭"
                    onClick={() => setAppearanceOpen(false)}
                  >
                ...
                <div className="cp-appearance-tabs" role="tablist" aria-label="自定义外观分类">
                  <button
                    type="button"
                    role="tab"
                    aria-selected={appearanceTab === "wallpaper"}
                    className={`cp-appearance-tab ${appearanceTab === "wallpaper" ? "is-active" : ""}`}
                    onClick={() => setAppearanceTab("wallpaper")}
                  >
                    Wallpaper
                  </button>
                  <button
                    type="button"
                    role="tab"
                    aria-selected={appearanceTab === "icons"}
                    className={`cp-appearance-tab ${appearanceTab === "icons" ? "is-active" : ""}`}
                    onClick={() => setAppearanceTab("icons")}
                  >
                    Icons
                  </button>
                </div>

                <div className="cp-appearance-modal-scroll">
                  <div
                    className="cp-appearance-tab-track"
                    style={{ transform: `translateX(${appearanceTab === "wallpaper" ? "0%" : "-50%"})` }}
                  >
                    <div className="cp-appearance-tab-panel" aria-hidden={appearanceTab !== "wallpaper"}>
                      <div className="cp-appearance-section cp-appearance-section--first cp-appearance-wallpaper-section">
                        <div className="cp-appearance-section-title">壁纸</div>
                        <div className="cp-appearance-wallpaper-preview">
                          {wallpaperUrlDraft ? (
                            <img
                              src={wallpaperUrlDraft}
                              alt=""
                              style={{
                                transform: `translate(-50%, -50%) translate(${(activeAppearance?.wallpaperOffsetX ?? 0)}%, ${(activeAppearance?.wallpaperOffsetY ?? 0)}%) scale(${(activeAppearance?.wallpaperScale ?? 100) / 100})`,
                              }}
                            />
                          ) : (
                        ...
                        <div className="cp-appearance-wallpaper-sliders">
                          <div className="cp-appearance-slider-row">
                            <span>缩放</span>
                            <input
                              type="range"
                              min={50}
                              max={300}
                              step={1}
                              value={activeAppearance?.wallpaperScale ?? 100}
                              onChange={(e) => applyWallpaperTransform({ wallpaperScale: Number(e.target.value) })}
                            />
                            <b>{activeAppearance?.wallpaperScale ?? 100}%</b>
                          </div>
                          <div className="cp-appearance-slider-row">
                            <span>水平位置</span>
                            <input
                              ...
                              max={100}
                              step={1}
                              value={activeAppearance?.wallpaperOffsetX ?? 0}
                              onChange={(e) => applyWallpaperTransform({ wallpaperOffsetX: Number(e.target.value) })}
                            />
                            <b>{activeAppearance?.wallpaperOffsetX ?? 0}</b>
                          </div>
                          <div className="cp-appearance-slider-row">
                            <span>垂直位置</span>
                            <input
                              ...
                              max={100}
                              step={1}
                              value={activeAppearance?.wallpaperOffsetY ?? 0}
                              onChange={(e) => applyWallpaperTransform({ wallpaperOffsetY: Number(e.target.value) })}
                            />
                            <b>{activeAppearance?.wallpaperOffsetY ?? 0}</b>
                          </div>
                          <button
                            type="button"
                            className="ui-btn cp-appearance-reset-transform-btn"
                            onClick={() => applyWallpaperTransform({ wallpaperScale: 100, wallpaperOffsetX: 0, wallpaperOffsetY: 0 })}
                          >
                            重置缩放/位置
                          </button>
                        </div>
                    ...
                    <div className="cp-appearance-tab-panel" aria-hidden={appearanceTab !== "icons"}>
                      <div className="cp-appearance-section cp-appearance-section--first">
                        <div className="cp-appearance-section-title">图标</div>
                        <div className="cp-appearance-icon-grid">
                          {manifest?.allAppIds.map((appId) => {
                            const spec = CHECKPHONE_APP_SPECS[appId];
                            const overrideUrl = activeAppearance?.iconOverrides?.[appId];
                            return (
                              <div key={appId} className="cp-appearance-icon-tile">
                                <div className="cp-appearance-icon-tile-frame">
                                  <label className="cp-appearance-icon-tile-btn" aria-label={`更换${spec.shortLabel ?? spec.label}图标`}>
                                    {renderAppIcon(appId, overrideUrl, { size: 30, strokeWidth: 1.5 })}
                                    <input
                                      type="file"
                                      accept="image/*"
                                      className="cp-appearance-file-input"
                                  ...
                                  {overrideUrl && (
                                    <button
                                      type="button"
                                      className="cp-appearance-icon-tile-clear"
                                      aria-label="恢复默认图标"
                                ...
                                <span className="cp-appearance-icon-tile-name">{spec.shortLabel ?? spec.label}</span>
                              </div>
                            );
                          })}
                        </div>
                      ...
                      <div className="cp-appearance-section">
                        <div className="cp-appearance-section-title">文字颜色</div>
                        <div className="cp-appearance-color-row">
                          {CHECKPHONE_LABEL_COLOR_PRESETS.map((color) => (
                            <button
                              key={color}
                              type="button"
                              className={`cp-appearance-color-swatch ${activeAppearance?.labelColor === color ? "is-active" : ""}`}
                          ...
                          <label
                            className={`cp-appearance-color-swatch cp-appearance-color-swatch--custom ${
                              activeAppearance?.labelColor && !CHECKPHONE_LABEL_COLOR_PRESETS.includes(activeAppearance.labelColor)
                                ? "is-active"
                                : ""
                            }`}
                            aria-label="自定义颜色"
                            style={activeAppearance?.labelColor ? { background: activeAppearance.labelColor } : undefined}
                          >
                            {!activeAppearance?.labelColor && <Palette size={13} strokeWidth={2.25} />}
                            <input
                              type="color"
                              className="cp-appearance-color-input"
                              value={activeAppearance?.labelColor ?? "#888888"}
                              onChange={(e) => applyLabelColor(e.target.value)}
                            />
                          </label>
                          <button type="button" className="cp-appearance-color-reset" onClick={() => applyLabelColor(null)}>
                            默认
                          </button>
                        </div>
                      </div>
                ...
                <div className="cp-desktop-settings-action cp-appearance-modal-action">
