...
// Decorative row icon removed — info rows no longer show a leading icon.
function DataSettingsIcon(_props: { icon: LucideIcon; color: string }) {
  return null;
}
...
/* ── Unified one-row-per-module list (Export & Import / Module Cleanup) ──
   No icon, no chip pill — a plain card list matching the toolbox rows:
   left = module name, right = its storage size. White when unselected,
   grey when selected; the whole row is clickable. */
type ModuleListItem = {
  id: DataModuleId;
  label: string;
  bytes?: number;
...
function ModuleListSelector({
  ...
  return (
    <div className="data-module-list" role="group" aria-label={ariaLabel}>
      {items.map((item) => {
        const selected = selectedIds.includes(item.id);
        return (
          <button
            key={item.id}
            type="button"
            className="data-module-list-row"
            aria-pressed={selected}
            {...(selected ? { "data-selected": "" } : {})}
            onClick={() => {
              if (selected) onChange(selectedIds.filter((id) => id !== item.id));
          ...
          >
            ...
            <span className="data-metric">{typeof item.bytes === "number" ? formatBytes(item.bytes) : "—"}</span>
          </button>
        );
      })}
    </div>
...
export function DataManagement({ onNotice }: DataManagementProps) {
  ...
  // Byte size per module, pulled from the live snapshot (was the standalone
  // "Module Breakdown" pie chart — now folded straight into each row below).
  const moduleBytesById = useMemo<Partial<Record<DataModuleId, number>>>(
    () => Object.fromEntries((snapshot?.modules ?? []).map((module: ModuleStats) => [module.moduleId, module.bytes])),
    [snapshot],
  );
  const moduleListItems = useMemo<ModuleListItem[]>(
    () => DATA_MODULES.map((module) => ({ id: module.id, label: module.label, bytes: moduleBytesById[module.id] })),
    [moduleBytesById],
  );
  const selectedExportBytes = useMemo(
    () => selectedExportModules.reduce((sum, id) => sum + (moduleBytesById[id] ?? 0), 0),
    [selectedExportModules, moduleBytesById],
  );
    ...
    <div className="page-menu data-management-menu" style={{ padding: 0 }}>
        ...
        <div className="menu-group">
          <div className="menu-item data-readonly-item">
            ...
            <div className="menu-label-group">
              <span className="menu-label font-medium menu-label-lg">浏览器持久化保护</span>
      ...
      <div className="data-section">
        ...
        <div className="menu-group">
          <div className="menu-item data-readonly-item">
            ...
            <div className="menu-label-group">
              <span className="menu-label font-medium menu-label-lg">图片/多媒体文件自动压缩及清理</span>
          ...
          <div className="menu-item data-readonly-item">
            <div className="menu-label-group">
              <span className="menu-label font-medium menu-label-lg">清理状态</span>
      ...
      <div className="data-section">
        ...
        <div className="menu-group">
          <div className="menu-item data-readonly-item">
            <div className="menu-label-group">
              <span className="menu-label font-medium menu-label-lg">本地导出·不含图片/多媒体</span>
          ...
          <div className="menu-item data-readonly-item">
            <div className="menu-label-group">
              <span className="menu-label font-medium menu-label-lg">导出模块</span>
              <span className="menu-desc">已选择 {selectedExportModules.length} / {DATA_MODULES.length} 个模块 · 共 {formatBytes(selectedExportBytes)}</span>
            </div>
            <div className="menu-right data-inline-actions">
              <button type="button" className="ui-btn-compact" onClick={() => setSelectedExportModules(ALL_MODULE_IDS)}>
                全选
              </button>
              <button type="button" className="ui-btn-compact" onClick={() => setSelectedExportModules(getLightModuleIds())}>
                轻量
              </button>
            </div>
          </div>
      ...
      <div className="data-section">
        ...
        <div className="menu-group">
          <div className="menu-item data-readonly-item">
            ...
            <div className="menu-label-group">
              <span className="menu-label font-medium menu-label-lg">备份到你的 Supabase</span>
          ...
          <div className="menu-item data-readonly-item">
            <div className="menu-label-group">
              <span className="menu-label font-medium menu-label-lg">自动备份</span>
      ...
      <div className="data-section data-cleanup-section">
        ...
        <div className="menu-group">
          <div className="menu-item data-readonly-item">
            <div className="menu-label-group">
              <span className="menu-label font-medium menu-label-lg">清理模块</span>
              <span className="menu-desc">已选择 {selectedClearModules.length} / {DATA_MODULES.length} 个模块，清理前会先尝试备份</span>
            </div>
            <div className="menu-right data-inline-actions">
              <button type="button" className="ui-btn-compact" onClick={() => setSelectedClearModules([])}>
                清空
              </button>
            </div>
          </div>
      ...
      {confirmRequest && (
        <ConfirmDialog
          ...
          icon={confirmRequest.type === "export" ? Upload : confirmRequest.type === "import" ? Download : confirmRequest.type === "media-maintenance" ? Archive : AlertTriangle}
