...
export function BindingManager() {
    ...
    // Top-right header icon — only on the global level, opens the character picker
    useEffect(() => {
        if (level === "global") {
            setSubpageRightAction("binding",
                <button
                    type="button"
                    onClick={() => setShowCharacterPicker(true)}
                    aria-label="为角色配置专属绑定"
            ...
            setSubpageRightAction("binding", null);
        }
        return () => setSubpageRightAction("binding", null);
    }, [level, setSubpageRightAction]);
    ...
    // Decorative field icons removed — binding cards no longer show a leading icon.
    const renderBindingFieldIcon = (_field: BindingField, _size = 23) => null;

    const renderAuxFieldIcon = (_field: AuxBindingField) => null;
            ...
            {level === "global" && (
                <>
                    ...
                    <section className="flex flex-col gap-2">
                        ...
                        <div className="binding-aux-card">
