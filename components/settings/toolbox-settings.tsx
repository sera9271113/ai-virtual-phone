...
import { Plus, SquarePlus, PackagePlus, Trash2, Search, Wrench, AlertCircle, MoreHorizontal, Upload, Download, ChevronDown, ChevronRight } from "lucide-react";
import type {
    CompositeToolConfig,
...
export function ToolboxSettings() {
    ...
    return (
        <div className="toolbox-menu flex flex-col gap-[24px] h-full">
            <input
                ref={importFileRef}
                type="file"
            ...
            />
            {/* REST Tools */}
            <div className="toolbox-section-title-row flex justify-between items-center gap-3">
            ...
            {restTools.length === 0 && restPackages.length === 0 ? (
                ...
                <div className="toolbox-card">
                    {singleRestTools.map(t => (
                        <div key={t.id} className="toolbox-row">
                            <button onClick={() => setEditRestId(t.id)}
                                className="flex-1 min-w-0 bg-none border-none cursor-pointer py-0 px-0 text-left flex items-center gap-2 overflow-hidden">
                                <div className="flex-1 flex flex-col gap-1 min-w-0">
                                    <div className="flex items-center gap-[6px] min-w-0">
                                        <span className="menu-label font-medium menu-label-lg truncate min-w-0">{t.name}</span>
                            ...
                            <Fragment key={pkg.id}>
                                <div className="toolbox-row">
                                    <button onClick={() => setEditRestPackageId(pkg.id)}
                                        ...
                                        <div className="flex-1 flex flex-col gap-1 min-w-0">
                                            <div className="flex items-center gap-[6px] min-w-0">
                                                <span className="menu-label font-medium menu-label-lg truncate min-w-0">{pkg.name}</span>
                                ...
                                {children.length === 0 ? (
                                    <div className="toolbox-row toolbox-row-child toolbox-row-empty">
                                        <span className="menu-desc !mt-0">暂无子工具</span>
                                    </div>
                                ) : children.map(t => (
                                    <div key={t.id} className="toolbox-row toolbox-row-child">
                                        <button onClick={() => setEditRestId(t.id)}
                                            className="flex-1 min-w-0 bg-none border-none cursor-pointer py-0 px-0 text-left flex items-center gap-2 overflow-hidden">
                                            <div className="flex-1 flex flex-col gap-1 min-w-0">
                                                <div className="flex items-center gap-[6px] min-w-0">
            ...
            {compositeTools.length === 0 && compositePackages.length === 0 ? (
                <div className="ui-empty-compact"><span className="menu-desc">暂无 Workflows</span></div>
            ) : (
                <div className="toolbox-card">
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                                {!t.builtIn && (
                                    <button onClick={() => { setConfirmDeleteId(t.id); setConfirmDeleteType("composite"); }}
                                        className="ui-link-btn opacity-40 hover:opacity-100" data-variant="danger"><Trash2 size={16} /></button>
                                )}
                                        ...
                                        {!pkg.builtIn && (
                                            <>
                                                <button onClick={() => addCompositeTool(pkg.id)}
                                                    className="ui-link-btn opacity-40 hover:opacity-100"><Plus size={16} /></button>
            ...
            <div className="toolbox-section-title-row flex justify-between items-center">
            ...
            <div className="toolbox-card">
                {internalCapabilities.map(item => {
                    const summary = (
                        <div className="flex-1 flex flex-col gap-1 min-w-0">
                            <div className="flex items-center gap-[6px] min-w-0">
                                <span className="menu-label font-medium menu-label-lg truncate min-w-0">{item.name}</span>
                    ...
                    return (
                        <div key={item.id} className="toolbox-row">
            ...
            {customAppTools.length === 0 ? (
                ...
                <div className="toolbox-card">
                    {customAppToolGroups.map(group => {
                        ...
                        if (group.length === 1) {
                            return (
                                <div key={customAppToolKey(first)} className="toolbox-row">
                                    <button
                                        type="button"
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 flex flex-col gap-1 min-w-0">
                                            <div className="flex items-center gap-[6px] min-w-0">
                                                <span className="menu-label font-medium menu-label-lg truncate min-w-0">{first.name}</span>
                        ...
                        return (
                            <Fragment key={first.appId}>
                                <div className="toolbox-row">
                                    <div className="flex-1 min-w-0 py-0 px-0 flex items-center gap-2 overflow-hidden">
                                        {first.appIconDataUrl && <img src={first.appIconDataUrl} alt="" className="w-8 h-8 rounded-[8px] object-cover shrink-0" />}
                                        <div className="flex-1 flex flex-col gap-1 min-w-0">
                                            <div className="flex items-center gap-[6px] min-w-0">
                                                <span className="menu-label font-medium menu-label-lg truncate min-w-0">{first.appName}工具</span>
                                                <span className="ui-badge shrink-0">APP</span>
                                ...
                                {group.map(tool => (
                                    <div key={customAppToolKey(tool)} className="toolbox-row toolbox-row-child">
                                        <button
                                            type="button"
                                        ...
                                        >
                                            <div className="flex-1 flex flex-col gap-1 min-w-0">
                                                <div className="flex items-center gap-[6px] min-w-0">
                                                    <span className="menu-label font-medium menu-label-lg truncate min-w-0">{tool.name}</span>
            ...
            {mcpServers.length === 0 ? (
                <div className="ui-empty-compact"><span className="menu-desc">暂无 MCP 服务器</span></div>
            ) : (
                <div className="toolbox-card">
                    {mcpServers.map(s => (
                        <div key={s.id} className="toolbox-row">
                            <button onClick={() => { setEditMcpId(s.id); setDiscoverError(null); setAuthResult(null); }}
                                className="flex-1 min-w-0 bg-none border-none cursor-pointer py-0 px-0 text-left flex items-center gap-2 overflow-hidden">
                                <div className="flex-1 flex flex-col gap-1 min-w-0">
                                    <span className="menu-label font-medium menu-label-lg truncate min-w-0">{s.name}</span>
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <div className="flex items-center gap-2">
                                    <input type="checkbox" id={directFetchInputId} checked={editRest.directFetch ?? true}
                                        ...
                                        style={{ accentColor: "var(--c-icon-active)" }} />
            ...
            {editMcp && (() => {
                ...
                const allowLocalNetworkInputId = `mcp-allow-local-${editMcp.id}`;
                ...
                return (
                    <ContentDialog title={isNewMcp ? "添加 MCP 服务器" : "MCP 服务器"} confirmLabel={isNewMcp ? "创建" : "完成"} onConfirm={onConfirm} onCancel={onCancel}>
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="flex items-center gap-2">
                                <input type="checkbox" id={allowLocalNetworkInputId} checked={editMcp.allowLocalNetwork ?? false}
                                    onChange={e => setM({ allowLocalNetwork: e.target.checked })}
                                    style={{ accentColor: "var(--c-icon-active)" }} />
                                <label htmlFor={allowLocalNetworkInputId} className="menu-desc">允许连接本机/内网地址（如 localhost、192.168.x.x）</label>
                            </div>
                            {editMcp.allowLocalNetwork && (
                                <span className="menu-desc ml-1 text-[var(--c-danger)]">
                                    已关闭该服务器的内网地址保护，仅在连接你自己部署、可信的本地 MCP 服务时开启。
                                </span>
                            )}
                            {editMcp.url.trim() && (
                                <>
                                    ...
                                    <label key={entry.key} className="ui-group-card !flex-row !items-center gap-3 py-2">
                                        ...
                                        <div className="flex min-w-0 flex-1 flex-col gap-1">
                                            <div className="flex items-center gap-1.5 min-w-0">
                                                <span className="menu-label font-medium menu-label-lg truncate min-w-0">{entry.label}</span>
