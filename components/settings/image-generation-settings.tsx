    ...
    return (
        <div className="flex flex-col gap-4 pb-8">
            ...
            <div className="stg-card stg-card--flush">
                <div className="stg-row" style={{ cursor: "default" }}>
                    <span className="flex-1 min-w-0 flex flex-col gap-0.5">
                        <span className="stg-row-label">启用自动生图</span>
                        <span className="text-[11px] text-[#999]">角色输出照片标签时自动调用</span>
                    </span>
            ...
            <div className="menu-group p-4 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1">
                    <label className="menu-desc ml-1">负面提示词</label>
                    <Textarea
                        value={settings.negativePrompt ?? ""}
                        onChange={(event) => updateSettings({ negativePrompt: event.target.value })}
                        placeholder="要求 LLM 在生图时避免的内容。"
                        rows={3}
                    />
                </div>
