...
import { ChatMessage, createOrGetSession, updateMessageMediaStatus, updateMessageMediaData, updateMessageMediaUrl, type ForwardedMessageSnapshot } from "@/lib/chat-storage";
...
export const MessageBubble = memo(function MessageBubble({ msg, onUpdate, charName, userName, onSystemMessage, groupSize, onShowDetail, characterId, onMusicPlay, onActionSelect, displayContent, defaultTranslationExpanded = false }: MessageBubbleProps) {
    switch (msg.mediaType) {
        ...
        case "merge_forward_card":
            return <MergeForwardCardBubble msg={msg} />;
...
function RedPacketBubble({ msg, charName, userName, groupSize, onShowDetail }: {
    ...
    const cardState = isDeclined ? "declined" : isDone ? "opened" : "sent";

    // 发出的红包始终显示底部横线区（未领取 / X/N已领取 / 已退回），
    // 保持卡片高度在各状态下一致；收到的红包维持原逻辑，只在多人红包
    // 已有人领取时才显示进度。
    const showFooter = isFromUser || (totalRecipients > 1 && claimedBy.length > 0);
    const footerText = isDeclined
        ? "已退回"
        : claimedBy.length > 0
            ? `${claimedBy.length}/${totalRecipients}已领取`
            : "未领取";

    return (
        <div
            className={`chat-media-card w-[206px] overflow-hidden cursor-pointer ${!isDone ? "red-packet-pulse" : ""}`}
            onClick={() => onShowDetail?.(msg)}
        >
            <div className="chat-media-mono-header p-3.5 flex flex-col justify-between gap-1.5 min-h-[100px]" data-state={cardState}>
                <div className="flex items-start justify-start">
                    <span className="chat-media-type-label flex items-baseline gap-1">
                        <span className="font-semibold" style={{ fontSize: "calc(18px*var(--app-text-scale,1))" }}>Red Packet</span>
                        <span className="font-semibold" style={{ fontSize: "calc(12px*var(--app-text-scale,1))" }}>红包</span>
                    </span>
                </div>
                <div className="flex-1 min-w-0">
                    {userShare != null ? (
                        <>
                            <div className="ts-20 font-bold chat-media-mono-title">¥{userShare.toFixed(2)}</div>
                            {d?.label && (
                                <div className="ts-14 mt-1 not-italic chat-media-mono-sub truncate">{d.label}</div>
                            )}
                        </>
                    ) : (
                        <div className="ts-13 font-medium chat-media-mono-title truncate">
                            {d?.label || "恭喜发财，大吉大利"}
                        </div>
                    )}
                    {isDeclined && !isFromUser && <div className="ts-12 mt-1 chat-media-mono-sub">已退回</div>}
                </div>
            </div>
            {showFooter && (
                <div className="ui-media-footer px-3.5 py-1.5 ts-11">
                    <span>{isFromUser ? footerText : `${claimedBy.length}/${totalRecipients}已领取`}</span>
                </div>
            )}
        </div>
    );
...
function TransferBubble({ msg, charName, userName, onShowDetail }: {
    ...
    const cardState = isDeclined ? "declined" : isReceived ? "opened" : "sent";

    // 发出的转账始终显示底部横线区（未领取 / 已收款 / 已退回），保持卡片
    // 高度在各状态下一致；收到的转账维持原逻辑，仅在已处理后显示状态行。
    let statusText = isReceived ? "已收款" : isDeclined ? "已退回" : "";
    const showFooter = isFromUser || Boolean(statusText);
    if (isFromUser && !statusText) statusText = "未领取";

    return (
        <div
            className="chat-media-card w-[206px] overflow-hidden cursor-pointer"
            onClick={() => onShowDetail?.(msg)}
        >
            <div className="chat-media-mono-header p-3.5 flex flex-col justify-between gap-1.5 min-h-[100px]" data-state={cardState}>
                <div className="flex items-start justify-start">
                    <span className="chat-media-type-label flex items-baseline gap-1">
                        <span className="font-semibold" style={{ fontSize: "calc(18px*var(--app-text-scale,1))" }}>Transfer</span>
                        <span className="font-semibold" style={{ fontSize: "calc(12px*var(--app-text-scale,1))" }}>转账</span>
                    </span>
                </div>
                <div className="flex-1">
                    <div className="ts-20 font-bold chat-media-mono-title">¥{d?.amount?.toFixed(2)}</div>
                    {d?.label && (
                        <div className="ts-14 mt-1 not-italic chat-media-mono-sub truncate">{d.label}</div>
                    )}
                </div>
            ...
            {d?.recipientName && (
                <div className="chat-media-mono-header px-3.5 py-1 ts-12 chat-media-mono-sub" data-state={cardState}>转给 {d.recipientName}</div>
            )}
            {showFooter && (
...
function PaymentRequestBubble({ msg, charName, userName, onShowDetail }: {
    ...
    const cardState = isDeclined ? "declined" : isPaid ? "opened" : "sent";

    return (
        <div
            className="chat-media-card w-[220px] overflow-hidden cursor-pointer"
            onClick={() => onShowDetail?.(msg)}
        >
            <div className="chat-media-mono-header p-3.5 flex items-start gap-2.5" data-state={cardState}>
                <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0 chat-media-mono-icon-circle">
                    <ReceiptText size={19} strokeWidth={1.5} />
                </div>
                <div className="flex-1 min-w-0 mt-2.5">
                    <div className="ts-11 chat-media-mono-sub truncate">{requester}发起代付请求</div>
                    <div className="ts-20 font-bold mt-1 chat-media-mono-title">¥{amount}</div>
                    <div className="ts-12 mt-1 chat-media-mono-sub line-clamp-2">{itemsText || "商品订单"}</div>
                </div>
            </div>
            <div className="ui-media-footer px-3.5 py-1.5 ts-12">
                <span>{statusText}</span>
...
function LocationBubble({ msg }: { msg: ChatMessage }) {
    ...
    return (
        <div
            className="chat-media-card w-[206px] overflow-hidden"
        >
            <div
                className="w-full h-[62px] flex items-center justify-center relative"
                style={{ background: "#f2f2f3" }}
            >
                <MapPin size={26} strokeWidth={1.5} className="relative z-[1]" style={{ color: "#9a9a9a" }} />
            </div>
            <div className="px-3 py-2.5 flex items-center gap-2" style={{ background: "#ffffff" }}>
                <MapPin size={15} strokeWidth={1.8} className="shrink-0 chat-media-mono-icon" />
                <span className="ts-13 chat-media-mono-title font-medium truncate">
                    {d?.label || "位置"}
                </span>
            </div>
        </div>
...
export function MediaDetailModal({ msg, userName, groupSize, onAccept, onClose }: MediaDetailModalProps) {
    ...
    // Header type label (top-left, italic)
    const headerTypeLabel = isRedPacket ? "红包" : isTransfer ? "转账" : "代付请求";
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="media-modal" onClick={(e) => e.stopPropagation()}>
                {/* Header: white/gray/black, no gradient, no emoji */}
                <div className="media-modal-header media-modal-header-mono">
                    <div className="media-modal-type-label">{headerTypeLabel}</div>
                ...
                <div className="media-modal-body">
                    ...
                    {canClaimRedPacket ? (
                        <div className="media-modal-actions">
                            <button onClick={handleRedPacketDecline} className="ui-btn ui-btn-media-outline">退回</button>
                            <button onClick={handleRedPacketAccept} className="ui-btn ui-btn-media-primary">领取</button>
                        </div>
                    ) : canActTransfer ? (
                        <div className="media-modal-actions">
                            <button onClick={handleTransferDecline} className="ui-btn ui-btn-media-outline">退回</button>
                            <button onClick={handleTransferAccept} className="ui-btn ui-btn-media-primary">收款</button>
                        </div>
                    ) : canActPaymentRequest ? (
                        <div className="media-modal-actions">
                            <button onClick={handlePaymentRequestDecline} className="ui-btn ui-btn-media-outline">拒绝</button>
                            <button onClick={handlePaymentRequestAccept} className="ui-btn ui-btn-media-primary">帮TA代付</button>
                        </div>
                    ) : statusText ? (
...
// ── Merge Forward Card (合并转发的聊天记录) ───────────────────────────────
function MergeForwardCardBubble({ msg }: { msg: ChatMessage }) {
    const d = msg.mediaData;
    const messages = d?.forwardedMessages || [];
    const title = d?.forwardTitle
        || (d?.forwardSourceType === "group"
            ? `${d?.forwardSourceName || "群聊"}的聊天记录`
            : (d?.forwardSourceName ? `${d.forwardSourceName}的聊天记录` : "合并转发的聊天记录"));
    const previewList = messages.slice(0, 3);
    const [detailOpen, setDetailOpen] = useState(false);

    return (
        <>
            <div
                className="chat-merge-forward-card"
                role="button"
                tabIndex={0}
                onClick={() => setDetailOpen(true)}
                onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") setDetailOpen(true); }}
            >
                <div className="chat-merge-forward-card-title">{title}</div>
                <div className="chat-merge-forward-card-preview">
                    {previewList.length > 0 ? previewList.map((item, idx) => (
                        <div className="chat-merge-forward-card-line" key={idx}>
                            <span className="chat-merge-forward-card-sender">{item.senderName}：</span>
                            <span className="chat-merge-forward-card-text">{item.mediaPreview || item.content || "[消息]"}</span>
                        </div>
                    )) : (
                        <div className="chat-merge-forward-card-line chat-merge-forward-card-empty">（暂无内容）</div>
                    )}
                </div>
                <div className="chat-merge-forward-card-footer">点击查看详情</div>
            </div>
            {detailOpen && typeof document !== "undefined" && createPortal(
                <MergeForwardDetailModal title={title} messages={messages} onClose={() => setDetailOpen(false)} />,
                document.body,
            )}
        </>
    );
...
function MergeForwardDetailModal({ title, messages, onClose }: {
    title: string;
    messages: ForwardedMessageSnapshot[];
    onClose: () => void;
}) {
    return (
        <div className="chat-merge-forward-detail-overlay" onClick={onClose}>
            <div className="chat-merge-forward-detail-dialog" onClick={(e) => e.stopPropagation()}>
                <div className="chat-merge-forward-detail-head">
                    <button
                        type="button"
                        className="chat-merge-forward-detail-close"
                        onClick={onClose}
                        aria-label="关闭"
                        title="关闭"
                    >
                    ...
                    <span className="chat-merge-forward-detail-title">{title}</span>
                    <span className="chat-merge-forward-detail-spacer" aria-hidden="true" />
                </div>
                <div className="chat-merge-forward-detail-body">
                    {messages.length === 0 ? (
                        <div className="chat-merge-forward-detail-empty">记录为空</div>
                    ) : messages.map((item, idx) => (
                        <div
                            className="chat-merge-forward-detail-row"
                            key={idx}
                            data-role={item.senderRole === "user" ? "user" : "other"}
                        >
                            <div className="chat-merge-forward-detail-meta">
                                <span className="chat-merge-forward-detail-sender">{item.senderName}</span>
                                <span className="chat-merge-forward-detail-time">{formatChatUiTime(item.createdAt)}</span>
                            </div>
                            <div className="chat-merge-forward-detail-bubble">{item.mediaPreview || item.content || "[消息]"}</div>
                        </div>
                    ))}
                </div>
            </div>
