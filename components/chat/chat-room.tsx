...
import { StickerPanel } from "./emoji-panel";
...
const ChatTextInputBar = memo(forwardRef<ChatTextInputHandle, {
    ...
    onSendText: (text: string, options?: { theater?: boolean }) => boolean;
    ...
    // 加号菜单只被"围观群"锁住；禁言时仍可打开，用来发系统指令，或开启番外指令模式后继续打字
    const plusMenuLocked = isSpectator;
    // 番外指令模式下，禁言不再锁文本框和发送——只有围观群依旧全锁
    const textLocked = isSpectator || (muteRemainingMs > 0 && !theaterMode);
    ...
    const handleSubmit = () => {
        ...
        if (!trimmed) return;
        if (!onSendText(trimmed, { theater: theaterMode })) return;
        setInputText("");
        resetTextareaHeight();
        onClosePanels();
    };

    const panelOpen = showStickerPanel || showPlusMenu;
    const plusIconColor = "var(--c-text)";
    ...
    const plusMenuItems = [
        ...
        { icon: <Phone size={30} strokeWidth={1.5} color={plusIconColor} />, label: "语音通话", onClick: onStartVoiceCall },
        { icon: <Video size={30} strokeWidth={1.5} color={plusIconColor} />, label: "视频通话", onClick: onStartVideoCall },
        { icon: <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke={plusIconColor} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="3" /><path d="M2 10h20" /></svg>, label: "红包", onClick: () => onOpenRichModal("red_packet") },
        { icon: <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke={plusIconColor} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><text x="12" y="16.5" textAnchor="middle" fontSize="12" fill={plusIconColor} stroke="none">¥</text></svg>, label: "转账", onClick: () => onOpenRichModal(isGroup ? "transfer_target" : "transfer") },
        // ── 第二页 ──
        { icon: <MapPin size={30} strokeWidth={1.5} color={plusIconColor} />, label: "定位", onClick: () => onOpenRichModal("location") },
        { icon: <Gift size={30} strokeWidth={1.5} color={plusIconColor} />, label: "礼物", onClick: () => onOpenRichModal("gift") },
        { icon: <ClipboardPenLine size={30} strokeWidth={1.5} color={plusIconColor} />, label: "系统指令", onClick: () => onOpenRichModal("system_instruction") },
        { icon: <Clapperboard size={30} strokeWidth={1.5} color={theaterMode ? plusIconActiveColor : plusIconColor} />, label: "番外指令模式", active: theaterMode, onClick: onToggleTheaterMode },
        ...customPlusActions.map(action => ({
            icon: action.appIconDataUrl
                ...
                : <Blocks size={30} strokeWidth={1.5} color={plusIconColor} />,
            label: action.label,
            onClick: () => onOpenCustomPlusAction(action),
        })),
    ];
    const plusMenuPages: (typeof plusMenuItems)[] = [];
    for (let i = 0; i < plusMenuItems.length; i += 8) plusMenuPages.push(plusMenuItems.slice(i, i + 8));
    const [plusPageIndex, setPlusPageIndex] = useState(0);
    const plusPagerRef = useRef<HTMLDivElement | null>(null);
    useEffect(() => { if (showPlusMenu) setPlusPageIndex(0); }, [showPlusMenu]);
    const handlePlusPagerScroll = useCallback(() => {
        const el = plusPagerRef.current;
        if (!el || el.clientWidth === 0) return;
        const idx = Math.round(el.scrollLeft / el.clientWidth);
        setPlusPageIndex(idx);
    }, []);
        ...
        <div className="chat-input-bar chat-room-main-pane flex flex-col" data-ui="input">
            ...
            <div className="chat-input-row">
                <button
                    ...
                    title="更多"
                >
                    <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="0.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="16" /><line x1="8" y1="12" x2="16" y2="12" /></svg>
                </button>
            ...
            {panelOpen && (
                <div className="chat-plus-menu-wrap">
                    {showStickerPanel ? (
                        <div className="chat-plus-menu-sticker-slot">
                            <StickerPanel
                        ...
                        <>
                            <div className="chat-plus-menu-pager" ref={plusPagerRef} onScroll={handlePlusPagerScroll}>
                                {plusMenuPages.map((page, pageIdx) => (
                                    <div className="chat-plus-menu" key={pageIdx}>
                                        {page.map((item, i) => (
                            ...
                            {plusMenuPages.length > 1 && (
                                <div className="chat-plus-menu-dots">
                                    {plusMenuPages.map((_, pageIdx) => (
                                        <span key={pageIdx} className="chat-plus-menu-dot" {...(pageIdx === plusPageIndex ? { "data-active": "" } : {})} />
                                    ))}
                                </div>
                            )}
...
export function ChatRoom({ session, onBack }: ChatRoomProps) {
    ...
    const [forwardTargetTab, setForwardTargetTab] = useState<"single" | "group">("single");
    const [forwardTargetSessionIds, setForwardTargetSessionIds] = useState<Set<string>>(new Set());
    ...
    useChatBottomReserve(
        wrapperRef,
        scrollRef,
        `${session.id}:${offlineMode}:${isMultiSelectMode}:${showStickerPanel}:${showPlusMenu}:${theaterMode}:${!!quotingMessage}`,
    );
    ...
    const handleAIMediaAction = (actionType: string, charN: string, userN: string) => {
        ...
        const updatedMediaData = targetMsg ? {
            ...
            // 底部"未领取"/"X/N已领取"文案只看 claimedBy，不看 status，
            // 只改 status 的话卡片会一直卡在"未领取"。
            ...(isRedPacketAccept ? {
                claimedBy: [...(targetMsg.mediaData?.claimedBy || []), charN],
                claimedAmounts: {
                    ...(targetMsg.mediaData?.claimedAmounts || {}),
                    [charN]: targetMsg.mediaData?.amount || 0,
                },
            } : {}),
    ...
    // QQ 式头衔徽标：群主/管理员身份实时计算（被踢/卸任后旧消息不再显示）；
    // 等级永远显示（LV0 起），自定义头衔优先于身份文案，颜色支持按成员自定义，未设置则按身份取默认色。
    const renderGroupRoleBadge = (senderCharacterId?: string) => {
        if (!session.isGroup || !senderCharacterId) return null;
        if (!isGroupMemberKey(session, senderCharacterId)) return null;
        const badgeText = getGroupBadgeText(session, senderCharacterId);
        const badgeColor = getGroupTitleColor(session, senderCharacterId);
        return (
            <span className="chat-role-badge" style={{ background: badgeColor }}>{badgeText}</span>
        );
    };
    ...
    // 围观群/被禁言时用户不能发言（番外指令模式下，禁言可以豁免，围观群依旧不行）
    const ensureGroupSpeakPermission = (options?: { allowMuted?: boolean }): boolean => {
        if (!session.isGroup) return true;
        if (session.isSpectator) {
            showChatToast("围观群不能发言，只能点生成");
            return false;
        }
        if (options?.allowMuted) return true;
    ...
    const handleSendText = (text: string, options?: { theater?: boolean }): boolean => {
        if (!ensureGroupSpeakPermission({ allowMuted: options?.theater })) return false;
        if (isGenerating) {
            showChatToast("请先等待对方回复");
            return false;
        }
        ...
        const menu = (
            <div
                ...
                <div className="flex">
                    ...
                    <button onClick={() => (m.role === "assistant" ? handleEditResponseStart(m) : handleEditMessageStart(m))} className="ctx-menu-btn">编辑</button>
                ...
                <div className="flex">
                    {m.role === "user" && (
                        <button onClick={() => handleRetractMessage(m.id)} className="ctx-menu-btn">撤回</button>
                    )}
    ...
    // ── 合并转发：按存储顺序取出被勾选的消息（不做删除那种"补齐空隙"，转发只取用户实际勾选的条目） ──
    const forwardOrderedMessages = useMemo(() => {
        if (selectedMessageIds.size === 0) return [];
        const storedMessages = loadChatMessages(session.id);
        return storedMessages.filter(msg => selectedMessageIds.has(msg.id));
    }, [selectedMessageIds, session.id]);

    const openForwardTargetPicker = useCallback(() => {
        if (forwardOrderedMessages.length === 0) {
            showChatToast("请选择要转发的消息");
            return;
        }
        setForwardTargetSessionIds(new Set());
        setForwardTargetTab(session.isGroup ? "group" : "single");
        setShowForwardTargetPicker(true);
    }, [forwardOrderedMessages.length, session.isGroup]);

    // 目标选择器数据源：单聊联系人 + 群聊会话，排除当前会话本身
    const forwardTargetOptions = useMemo(() => {
        const allSessions = loadChatSessions().filter(s => s.id !== session.id);
        const chars = loadCharacters();
        const charById = new Map(chars.map(c => [c.id, c]));

        const single = allSessions
            .filter(s => !s.isGroup)
            .map(s => ({
                sessionId: s.id,
                name: s.alias || charById.get(s.contactId)?.name || "未知联系人",
                avatar: charById.get(s.contactId)?.avatar || null,
            }));

        const group = allSessions
            .filter(s => s.isGroup)
            .map(s => ({
                sessionId: s.id,
                name: s.groupName || "群聊",
    ...
    const toggleForwardTargetSession = useCallback((sessionId: string) => {
        setForwardTargetSessionIds(prev => {
            const next = new Set(prev);
    ...
    const buildForwardedMessageSnapshots = useCallback((msgs: ChatMessage[]): ForwardedMessageSnapshot[] => {
        return msgs.map(m => ({
            senderName: m.role === "user" ? (userIdentity?.name || "你") : (m.senderName || character?.name || "对方"),
            senderRole: m.role,
            senderCharacterId: m.role === "user" ? undefined : (m.senderCharacterId || character?.id),
            content: m.content,
            mediaType: m.mediaType,
            mediaPreview: m.mediaType ? (getChatMessagePreview(m) || undefined) : undefined,
            createdAt: m.createdAt,
        }));
    }, [character?.id, character?.name, userIdentity?.name]);

    const confirmForwardToTargets = useCallback(() => {
        if (forwardOrderedMessages.length === 0 || forwardTargetSessionIds.size === 0) return;
        setIsForwarding(true);
        try {
            const snapshots = buildForwardedMessageSnapshots(forwardOrderedMessages);
            const sourceIsGroup = !!session.isGroup;
            const sourceName = sourceIsGroup
                ? (session.groupName || "群聊")
                : `${userIdentity?.name || "你"}与${character?.name || "对方"}`;
            const forwardId = `fwd_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;

            const allTargets = [...forwardTargetOptions.single, ...forwardTargetOptions.group]
                .filter(t => forwardTargetSessionIds.has(t.sessionId));
            ...
            allTargets.forEach(target => {
                pushChatMessage({
                    ...
                    role: "user",
                    content: "",
                    mediaType: "merge_forward_card",
                    mediaData: {
                        forwardId,
                        forwardSourceType: sourceIsGroup ? "group" : "single",
                        forwardSourceName: sourceName,
                        forwardTitle: sourceIsGroup ? `${sourceName}的聊天记录` : "合并转发的聊天记录",
            ...
            cancelMultiSelect();
            showChatToast(`已转发给 ${allTargets.length} 个会话`);
        } finally {
            setIsForwarding(false);
        }
    }, [forwardOrderedMessages, forwardTargetSessionIds, forwardTargetOptions, buildForwardedMessageSnapshots, session.isGroup, session.groupName, userIdentity?.name, character?.name, cancelMultiSelect]);
        ...
        <div ref={wrapperRef} className={`session-${session.id} chat-room-wrapper page-shell inset-0 flex flex-col z-20`} style={chatRoomBackgroundStyle} {...(bgLoading ? { "data-loading": "" } : {})} {...(bgImageResolved ? { "data-has-bg-image": "" } : {})} {...(showSettings ? { "data-settings-open": "" } : {})}>
            ...
            <header className="page-header chat-room-main-pane" data-ui="header">
                ...
                <div className="page-header-content">
                    ...
                    <span className="page-title chat-room-title-row">
                    ...
                    <span className="page-header-right chat-room-header-right">
                        <button
                            className="page-back-btn chat-offline-toggle-btn"
                            type="button"
                            onClick={toggleOfflineMode}
                            disabled={isOfflineGenerating}
                            aria-label={offlineMode ? "返回线上模式" : "进入线下模式"}
                            title={offlineMode ? "返回线上模式" : "进入线下模式"}
                        >
                        ...
                        <button className="page-back-btn chat-header-more-btn" type="button" onClick={() => setShowSettings(true)} aria-label="更多">
                ...
                {offlineMode && (
                    <div className="chat-offline-body">
                        ...
                        {visibleOfflineTurns.map((turn, turnIdx) => {
                            ...
                            {showTime && <div className="chat-offline-time">{formatChatUiTime(turn.createdAt, session.timestampShowSeconds)}</div>}
                                        ...
                                        {groupMessages.map((gMsg) => (
                                            uiRole(gMsg) === "system" ? (
                                                ...
                                                <div key={gMsg.id} className={`flex ${gMsg.role === "user" ? "justify-end" : "justify-start"}`}>
                                                    <div className={`chat-msg-content-wrap flex flex-col min-w-0 max-w-[75%] ${session.isGroup && gMsg.role !== "user" ? "has-sender-name" : ""}`}>
                                                        {session.isGroup && gMsg.role !== "user" && (
                                                            <span className="chat-group-sender-name">
                                                                {gMsg.senderName || ""}
                                                                {renderGroupRoleBadge(gMsg.senderCharacterId)}
                                                            </span>
                                                        )}
                                                        <div
                                                            onPointerDown={(e) => { e.stopPropagation(); handleMessagePointerDown(e, gMsg.id); }}
                            ...
                            {showTime && (
                                <div className="flex justify-center w-full">
                                    <span className="chat-sys-msg py-[2px] px-2 rounded select-none">
                                        {formatChatUiTime(msg.createdAt, session.timestampShowSeconds)}
                                    </span>
                                </div>
                            )}
                            <div
                                ...
                                {uiRole(msg) === "system" ? (
                                    ...
                                    <>
                                        ...
                                        {!isSilentThought && !isEmptyBubble && <div
                                            className={`chat-msg-content-wrap flex flex-col min-w-0 max-w-[70%] ${isStandaloneHtmlPreview ? "chat-msg-content-wrap-html" : ""} ${session.isGroup && msg.role !== "user" ? "has-sender-name" : ""}`}
                            ...
                            {hasFoldedPanel && expandedMonologueId === msg.id && (
                                <div className="chat-thought-card">
                                    ...
                                    <div className="chat-thought-title">
                                        💭 {renderMsg.innerMonologue ? "偷窥TA的内心" : "状态栏"}
                                    </div>
                                    {/* State values panel */}
            ...
            <ChatTextInputBar
                ...
                onClosePanels={() => { setShowStickerPanel(false); setShowPlusMenu(false); }}
	                onToggleStickerPanel={() => { setShowStickerPanel(!showStickerPanel); setShowPlusMenu(false); }}
	                onTogglePlusMenu={() => { setShowPlusMenu(!showPlusMenu); setShowStickerPanel(false); }}
            ...
            {showForwardTargetPicker && (
                <div className="chat-forward-target-overlay" onClick={() => setShowForwardTargetPicker(false)}>
                    <div className="chat-forward-target-dialog" onClick={(e) => e.stopPropagation()}>
                        <div className="chat-forward-target-head">
                            <span className="chat-forward-target-title">选择要转发到的会话</span>
                            <button
                                type="button"
                                className="chat-forward-target-close"
                        ...
                        <div className="chat-forward-target-tabs">
                            <button
                                type="button"
                                className="chat-forward-target-tab"
                                data-active={forwardTargetTab === "single" ? "" : undefined}
                                onClick={() => setForwardTargetTab("single")}
                            >
                                单人聊天
                            </button>
                            <button
                                type="button"
                                className="chat-forward-target-tab"
                                data-active={forwardTargetTab === "group" ? "" : undefined}
                                onClick={() => setForwardTargetTab("group")}
                            >
                                群聊
                            </button>
                        </div>
                        <div className="chat-forward-target-list">
                            {(forwardTargetTab === "single" ? forwardTargetOptions.single : forwardTargetOptions.group).length === 0 ? (
                                <div className="chat-forward-target-empty">
                                    {forwardTargetTab === "single" ? "暂无其他单人聊天" : "暂无群聊"}
                                </div>
                            ) : (forwardTargetTab === "single" ? forwardTargetOptions.single : forwardTargetOptions.group).map(target => (
                                <div
                                    key={target.sessionId}
                                    className="chat-forward-target-item"
                                    {...(forwardTargetSessionIds.has(target.sessionId) ? { "data-selected": "" } : {})}
                                    onClick={() => toggleForwardTargetSession(target.sessionId)}
                                >
                                    <span className="chat-forward-target-avatar">
                                        {target.avatar ? <img src={target.avatar} alt="" /> : <ChatFallbackAvatar />}
                                    </span>
                                    <span className="chat-forward-target-name">{target.name}</span>
                                    <span className="chat-forward-target-check" aria-hidden="true">
                                        {forwardTargetSessionIds.has(target.sessionId) && <Check size={12} strokeWidth={2.5} />}
                                    </span>
                                </div>
                            ))}
                        </div>
                        <div className="chat-forward-target-footer">
                            <button
                                type="button"
                                className="chat-forward-target-send-btn"
                                disabled={forwardTargetSessionIds.size === 0 || isForwarding}
                                onClick={confirmForwardToTargets}
                            >
                                {isForwarding ? "发送中…" : `发送${forwardTargetSessionIds.size > 0 ? `(${forwardTargetSessionIds.size})` : ""}`}
                            </button>
                        </div>
                    </div>
                </div>
