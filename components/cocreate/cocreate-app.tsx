...
const CAST_COLOR_SWATCHES = ["#1A1A1A", "#696969", "#8A8A8A", "#FFFFFF", "#8A8077", "#94A177"];
...
export function CoCreateApp({ onClose, onNotice }: CoCreateAppProps) {
  ...
  const [messageDeleteTargetId, setMessageDeleteTargetId] = useState<string | null>(null);
  ...
  const messageDeleteTarget = messageDeleteTargetId ? session.messages.find((message) => message.id === messageDeleteTargetId) || null : null;
  ...
  function deleteMessage(message: CoCreateSession["messages"][number]): void {
    if (isGenerating || isArchiving) return;
    persistSession({
      ...session,
      messages: session.messages.filter((item) => item.id !== message.id),
    });
    setMessageDeleteTargetId(null);
    setStatus("已删除该条消息。");
  }
  ...
  return (
    <div className="cocreate-app">
            ...
            editingChapter && !chapterReaderEditing ? (
              <>
                ...
                <button
                  type="button"
                  className="cocreate-icon-button"
                  onClick={() => setChapterDeleteTargetId(editingChapter.id)}
                  aria-label={`删除第 ${editingChapter.num} 章`}
                  title="删除章节"
                >
                  <Trash2 size={15} />
                </button>
                  ...
                  <article key={message.id} className={isUser ? "cocreate-write-block cocreate-write-user" : "cocreate-write-block"}>
                    <div className="cocreate-block-meta">
                      ...
                      <span className="cocreate-message-actions" aria-label="消息操作">
                        ...
                        <button
                          type="button"
                          onClick={() => setMessageDeleteTargetId(message.id)}
                          disabled={isGenerating || isArchiving}
                          aria-label="删除这条消息"
                          title="删除"
                        >
      ...
      {messageDeleteTarget && (
        <div className="cocreate-modal-backdrop" role="presentation">
          <section className="cocreate-archive-dialog" role="dialog" aria-modal="true" aria-labelledby="cocreate-message-delete-title">
            <span>DELETE MESSAGE</span>
            <h2 id="cocreate-message-delete-title">删除这条消息？</h2>
            <p>这条{messageDeleteTarget.role === "user" ? "用户" : "AI"}消息会被永久删除，无法恢复。</p>
            <div className="cocreate-archive-actions">
              <button type="button" onClick={() => setMessageDeleteTargetId(null)}>取消</button>
              <button type="button" className="cocreate-danger-action" onClick={() => deleteMessage(messageDeleteTarget)}>
                确认删除
              </button>
            </div>
          </section>
