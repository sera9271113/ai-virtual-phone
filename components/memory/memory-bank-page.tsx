...
import { generateEmbedding, resolveEmbeddingModel, estimateMemorySignals } from "@/lib/memory-embedding";
...
// All memory badges now share the same gray look as the old "重要度" (action) tag,
// with a larger, fuller capsule radius.
                ...
                {entries.length === 0 ? (
                    ...
                    entries.map(entry => (
                        ...
                        >
                            ...
                            <div className="mem-report-badges" style={memoryBadgeRowStyle}>
                                {(() => {
                                    const badge = formatMemoryBadge(entry);
                                    return (
                                        <>
                                            <span className="ui-status-tag" data-variant="action" style={memoryBadgeCapsuleStyle}>
                                                {badge.status === "active" ? "活跃" : badge.status === "sleeping" ? "沉睡" : "归档"}
                                            </span>
                                            <span className="ui-status-tag" data-variant="action" style={memoryBadgeCapsuleStyle}>重要度: {badge.importance}</span>
                                            <span className="ui-status-tag" data-variant="action" style={memoryBadgeCapsuleStyle}>情绪正负: {badge.valence}</span>
                                            <span className="ui-status-tag" data-variant="action" style={memoryBadgeCapsuleStyle}>情绪强度: {badge.arousal}</span>
                                            <span className="ui-status-tag" data-variant="action" style={memoryBadgeCapsuleStyle}>唤醒次数: {badge.readCount}次</span>
                                            <span className="ui-status-tag" data-variant="action" style={memoryBadgeCapsuleStyle}>上次唤醒: {badge.lastReadAt}</span>
                                        </>
                                    );
                                })()}
                            </div>
