...
import { generateEmbedding, resolveEmbeddingModel, estimateMemorySignals } from "./memory-embedding";
...
export async function runSummarizationPipeline(
    ...
    // Estimate importance/valence/arousal for the summary via a lightweight LLM call.
    // Reuses the same summarization API config; fails safe to {importance: 8, valence: 0,
    // arousal: 0} on error, preserving the old fixed-importance behavior as a fallback.
    const signals = await estimateMemorySignals(summary, apiConfig, { fallbackImportance: 8 });
