...
/**
 * Save one entry and delete a set of others in a single IndexedDB transaction.
 * Used for "合并" (merge): writing the merged entry and removing its source
 * entries must be all-or-nothing — doing them as two separate open/close cycles
 * (as saveMemoryEntry + deleteMemoryEntries would) leaves a window where the
 * merged entry could be written but the originals never removed (or vice versa)
 * if anything interrupts the app in between, which looks to the user like
 * "nothing happened" even though it silently did half the job.
 */
export async function saveMemoryEntryAndDelete(entry: MemoryEntry, deleteIds: string[]): Promise<void> {
    const db = await openDb();
    if (!db) return;
    try {
        const tx = db.transaction(STORE_NAME, "readwrite");
        ...
        for (const id of deleteIds) {
            if (id === entry.id) continue; // guard against deleting the entry we just wrote
            store.delete(id);
        }
        await new Promise<void>((res, rej) => {
            tx.oncomplete = () => res();
