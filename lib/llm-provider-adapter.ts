...
import type { ApiConfig, PresetConfig, GenerationParams } from "./settings-types";
import { DEFAULT_GENERATION_PARAMS } from "./settings-types";
import {
...
/** Resolve effective generation params: API-level > preset-level > defaults */
function resolveGenParams(config: ApiConfig, preset: PresetConfig | null): GenerationParams {
    const gp = config.generationParams;
    return {
        temperature:        gp?.temperature        ?? preset?.temperature        ?? DEFAULT_GENERATION_PARAMS.temperature,
        top_p:              gp?.top_p              ?? preset?.top_p              ?? DEFAULT_GENERATION_PARAMS.top_p,
        top_k:              gp?.top_k              ?? preset?.top_k              ?? DEFAULT_GENERATION_PARAMS.top_k,
        frequency_penalty:  gp?.frequency_penalty  ?? preset?.frequency_penalty  ?? DEFAULT_GENERATION_PARAMS.frequency_penalty,
        presence_penalty:   gp?.presence_penalty   ?? preset?.presence_penalty   ?? DEFAULT_GENERATION_PARAMS.presence_penalty,
        repetition_penalty: gp?.repetition_penalty ?? preset?.repetition_penalty ?? DEFAULT_GENERATION_PARAMS.repetition_penalty,
        openai_max_tokens:  gp?.openai_max_tokens  ?? preset?.openai_max_tokens  ?? DEFAULT_GENERATION_PARAMS.openai_max_tokens,
        min_p:              gp?.min_p              ?? preset?.min_p,
        top_a:              gp?.top_a              ?? preset?.top_a,
    };
}

function buildSamplingBody(config: ApiConfig, preset: PresetConfig | null): Record<string, unknown> {
    const gp = resolveGenParams(config, preset);
    ...
    if (gp.openai_max_tokens > 0) body.max_tokens = gp.openai_max_tokens;
    if (gp.repetition_penalty !== 1) body.repetition_penalty = gp.repetition_penalty;
    if (gp.top_k > 0) body.top_k = gp.top_k;
    if (gp.min_p && gp.min_p > 0) body.min_p = gp.min_p;
    if (gp.top_a && gp.top_a > 0) body.top_a = gp.top_a;
    return body;
}
...
function buildAnthropicRequest(
    ...
    const body: Record<string, unknown> = {
        ...
        temperature: gp.temperature,
        max_tokens: gp.openai_max_tokens > 0 ? gp.openai_max_tokens : ANTHROPIC_AUTO_MAX_TOKENS,
    };
    if (gp.top_p !== undefined) body.top_p = gp.top_p;
    if (gp.top_k > 0) body.top_k = gp.top_k;
    ...
    const body: Record<string, unknown> = {
        ...
        generationConfig: (() => {
            ...
            return {
                temperature: gp.temperature,
                topP: gp.top_p,
                ...(gp.top_k > 0 ? { topK: gp.top_k } : {}),
                ...(gp.openai_max_tokens > 0 ? { maxOutputTokens: gp.openai_max_tokens } : {}),
            };
        })(),
        safetySettings: [
