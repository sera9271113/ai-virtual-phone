...
import type { StateValue, ForwardedMessageSnapshot } from "./chat-storage";
...
export interface AssemblerInput {
    ...
    htmlGenerationInstruction?: string;      // session-specific HTML-generation output rule for {{htmlGenerationInstruction}}
...
export function assemblePromptPayload(input: AssemblerInput): LLMMessage[] {
    ...
    if (hasPromptOrder) {
        ...
        engine.htmlGenerationInstruction = input.htmlGenerationInstruction ?? "";
...
// ── Merge Forward Card → Structured Prompt Injection ──────────────────────
// Converts a merge_forward_card message's full snapshot list into structured
// plain text so it can be injected into the target chat's AI prompt context.
// In group chats every member reads the same shared `history` array, so a
// single injected block here is automatically visible to all AI roles —
// satisfying "群聊多角色记忆共享" without any per-member special-casing.
function formatForwardedSnapshotLine(snap: ForwardedMessageSnapshot): string {
    const text = (snap.mediaType && snap.mediaPreview) ? snap.mediaPreview : (snap.content || "[消息]");
    return `${snap.senderName}：${text}`;
}

export function formatMergeForwardCardForHistory(msg: ChatMessage): string {
    ...
    const snapshots = d?.forwardedMessages || [];
    if (snapshots.length === 0) {
        return `[合并转发的聊天记录：${title}]（记录为空）`;
    }
    return [
        `[合并转发的聊天记录：${title}，以下是被转发的完整历史对话，是一段共享的聊天记录，可作为共同的记忆参考，其中出现的人名不代表当前对话的参与者]`,
        ...snapshots.map(formatForwardedSnapshotLine),
        `[聊天记录结束]`,
    ].join("\n");
}
...
export function formatRichMediaForHistory(msg: ChatMessage, userName: string, charName: string, isGroup?: boolean): string {
    ...
    switch (msg.mediaType) {
        ...
        case "merge_forward_card":
            return formatMergeForwardCardForHistory(msg);
        case "audio":
...
export interface GroupAssemblerInput {
    ...
    htmlGenerationInstruction?: string;
...
function pushGroupChronologicalShortTermBlocks(params: {
    ...
    items.forEach((item, idx) => {
        ...
        // 合并转发卡片：content 为空，需转换为结构化文本才能进入短期记忆时间线
        if (msg.mediaType === "merge_forward_card") {
            body = formatMergeForwardCardForHistory(msg);
        }
...
export function assembleGroupPromptPayload(input: GroupAssemblerInput): LLMMessage[] {
    ...
    if (hasPromptOrder) {
        ...
        groupEngine.htmlGenerationInstruction = input.htmlGenerationInstruction ?? "";
    ...
    if (useChronologicalShortTerm) {
        ...
        history.forEach((msg, idx) => {
            ...
            // 合并转发卡片：content 为空，需转换为结构化文本注入共享历史，
            // 以便群内所有 AI 角色都能读取到这段被转发的记忆
            if (msg.mediaType === "merge_forward_card") {
                body = formatMergeForwardCardForHistory(msg);
            }
