...
export async function createBackupBlob(moduleIds?: DataModuleId[], options: BackupOptions = {}): Promise<{ blob: Blob; manifest: BackupManifest }> {
  ...
  // Stream one module at a time — avoids the double-read from inspectData()
  // and keeps peak memory low (same approach as cloud backup).
  const manifestModules: { id: DataModuleId; label: string; records: number; bytes: number }[] = [];
  for (const dataModule of selectedModules) {
    const { payload, records } = await buildSingleModulePayload(dataModule, options, collector);
    const json = JSON.stringify(payload);
    zip.file(`modules/${payload.moduleId}.json`, json);
    const definition = DATA_MODULES.find((d) => d.id === dataModule.id);
    manifestModules.push({
      id: dataModule.id,
      label: definition?.label ?? dataModule.id,
      records,
      bytes: json.length,
    });
  }
  ...
  const manifest: BackupManifest = {
    format: "ai-phone-backup",
    version: collector.media.size > 0 ? 2 : 1,
    createdAt: new Date().toISOString(),
    origin: typeof window !== "undefined" ? window.location.origin : "",
    modules: manifestModules,
    totalBytes: manifestModules.reduce((sum, m) => sum + m.bytes, 0),
    totalRecords: manifestModules.reduce((sum, m) => sum + m.records, 0),
    ...(options.excludeMedia ? { mediaExcluded: true } : {}),
  };
