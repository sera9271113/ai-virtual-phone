...
export type ChatSession = {
    ...
    htmlGenerationEnabled?: boolean;   // 默认 false（不同于双语翻译默认开，这个建议默认关）
    htmlGenerationPrompt?: string;     // 可选：允许用户自定义指令文案，留空则用默认文案
    timestampShowSeconds?: boolean;    // 消息气泡时间戳是否精确到秒，默认 false
    timeZone?: string;                 // 会话级时区覆盖（IANA），优先级高于 character.timeZone；留空回退角色时区，再空则兜底 Asia/Shanghai
    nativeExpandedToolSourceIds?: string[];
    visionImagePromptLimit?: number;
    // Group chat fields
    isGroup?: boolean;
    groupName?: string;
    groupAvatar?: string; // Custom group avatar (data URL); falls back to composite member-avatar grid when unset
    ...
    // Group title / activity fields ("self" = the user, otherwise characterId)
    groupActivity?: Record<string, number>; // member key → 累计活跃度（按该成员在本群历史消息的 token 数累加）
    groupTitles?: Record<string, string>; // member key → 自定义头衔文字（由群主/管理员设置）
    groupTitleColors?: Record<string, string>; // member key → 自定义头衔徽标颜色（hex），未设置则按身份取默认色
};
...
// 合并转发：被转发消息的只读快照，独立于原会话存储，脱离原消息也能正常展示
...
export type ChatMessage = {
    ...
    mediaData?: {
        ...
        forwardId?: string;        // 合并转发批次 ID
        forwardSourceType?: "single" | "group"; // 转发来源会话类型
        forwardSourceName?: string; // 转发来源会话显示名
        forwardTitle?: string;     // 合并转发卡片标题（未设置时按来源类型/名称拼接）
        forwardedMessages?: ForwardedMessageSnapshot[]; // 被转发消息的只读快照列表
    };
...
export type ChatAppSettings = {
    ...
    /** Settings page avatar (base64 data-url from local photo album) */
    settingsAvatar?: string;
    /** Settings page custom display ID */
    settingsCustomId?: string;
};
...
export function pushChatMessage(msg: Omit<ChatMessage, "id" | "createdAt" | "status"> & { status?: ChatMessageStatus }): ChatMessage {
    ...
    if (sessIdx !== -1) {
        ...
        // 群聊活跃度：按发言人 key（"self" = 用户，否则 characterId）累加本条消息的 token 数。
        // 只在这里统一埋点，覆盖所有发送入口，不需要在每个发送入口分别处理。
        if (session.isGroup && (newMsg.role === "user" || newMsg.role === "assistant")) {
            const activityKey = newMsg.role === "user" ? "self" : newMsg.senderCharacterId;
            const tokens = activityKey ? estimateTokens(newMsg.content) : 0;
            if (activityKey && tokens > 0) {
                const activity = { ...(session.groupActivity || {}) };
                activity[activityKey] = (activity[activityKey] || 0) + tokens;
                session.groupActivity = activity;
                sessionChanged = true;
            }
        }
        ...
        if (isSessionPreviewCandidate(newMsg)) {
            ...
            session.updatedAt = newMsg.createdAt;
            sessionChanged = true;
        }
...
// ── Saved custom colors for group title badges (global, reused across all chats) ───
const GROUP_TITLE_COLOR_PALETTE_KEY = "chat_group_title_color_palette";
const MAX_GROUP_TITLE_COLOR_PALETTE = 16;

export function loadGroupTitleColorPalette(): string[] {
    if (typeof window === "undefined") return [];
    try {
        const raw = kvGet(GROUP_TITLE_COLOR_PALETTE_KEY);
...
/** 把一个颜色加入调色板（去重、置顶、限制最大数量），返回更新后的调色板。 */
export function saveGroupTitleColorToPalette(color: string): string[] {
    if (typeof window === "undefined") return [];
    const trimmed = color.trim();
    if (!trimmed) return loadGroupTitleColorPalette();
    const existing = loadGroupTitleColorPalette().filter(c => c.toLowerCase() !== trimmed.toLowerCase());
    const next = [trimmed, ...existing].slice(0, MAX_GROUP_TITLE_COLOR_PALETTE);
    kvSet(GROUP_TITLE_COLOR_PALETTE_KEY, JSON.stringify(next));
    return next;
}

export function removeGroupTitleColorFromPalette(color: string): string[] {
    if (typeof window === "undefined") return [];
    const next = loadGroupTitleColorPalette().filter(c => c.toLowerCase() !== color.toLowerCase());
    kvSet(GROUP_TITLE_COLOR_PALETTE_KEY, JSON.stringify(next));
    return next;
}
...
export function updateChatMessage(
    messageId: string,
    patch: Partial<Pick<ChatMessage, "role" | "content" | "mediaType" | "mediaUrl" | "mediaData" | "rawResponseText" | "responseBatchId" | "responseRoundId">>,
): ChatMessage | null {
    const idx = _messagesCache.findIndex(m => m.id === messageId);
