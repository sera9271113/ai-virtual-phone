...
export type CharacterWorldGroup = {
    ...
    createdAt: string;
    updatedAt: string;
    color: string;          // 文件夹本身的颜色（图标/开合视觉着色用），与角色卡片色是两套独立的色轮，互不影响
};

/** 文件夹兜底色：淡灰色，未自定义时所有文件夹都长这样 */
export const DEFAULT_WORLD_FOLDER_COLOR = "#d8dadd";

function generateDefaultWorldColor(_seed: string): string {
    return DEFAULT_WORLD_FOLDER_COLOR;
}
...
function createDefaultGroup(memberIds: string[], now = new Date().toISOString()): CharacterWorldGroup {
    return {
        ...
        createdAt: now,
        updatedAt: now,
        color: generateDefaultWorldColor(DEFAULT_CHARACTER_WORLD_ID),
    };
}
...
function normalizeGroups(groups: CharacterWorldGroup[], characters: Character[]): { groups: CharacterWorldGroup[]; changed: boolean } {
    ...
    let normalized = groups
        ...
        .map((group): CharacterWorldGroup => {
            ...
            const validColor = typeof group.color === "string" && /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(group.color.trim());
            if (!validColor) changed = true; // 旧数据没有 color 字段：首次读取时补全一个默认色并落盘
            ...
            return {
                ...
                color: validColor ? group.color.trim() : generateDefaultWorldColor(group.id),
            };
        });
...
export function createCharacterWorldGroup(name: string, color?: string): CharacterWorldGroup {
    const groups = loadCharacterWorldGroups();
    const now = new Date().toISOString();
    const id = generateId("world");
    const validColor = typeof color === "string" && /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(color.trim());
    const group: CharacterWorldGroup = {
        ...
        createdAt: now,
        updatedAt: now,
        color: validColor ? (color as string).trim() : generateDefaultWorldColor(id),
    };
...
export function updateCharacterWorldColor(groupId: string, color: string): void {
    if (!/^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(color.trim())) return;
    const now = new Date().toISOString();
    saveCharacterWorldGroups(loadCharacterWorldGroups().map(group =>
        group.id === groupId
            ? { ...group, color: color.trim(), updatedAt: now }
            : group
    ));
}
