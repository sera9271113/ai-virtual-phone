...
export async function generateStoryCompletion(
  ...
  const meta = { characterName: character.name };
  ...
  // sendLLMRequest (non-streaming) and sendLLMStreamRequest (SSE) are two separate
  // code paths in chat-engine.ts — sendLLMRequest doesn't know about `stream`/`onDelta`
  // at all, so streaming has to actually call the dedicated stream function.
  let rawOutput: string;
  if (options?.stream) {
    let accumulated = "";
    let reasoningAccumulated = "";
    const streamResult = await sendLLMStreamRequest(apiConfig, preset, llmMessages, regexes, meta, requestOptions, {
      onDelta: (delta) => {
        accumulated += delta;
    ...
    // sendLLMStreamRequest doesn't fold reasoning into `content` the way the
    // non-streaming sendLLMRequest does — do it ourselves so parseStoryResponse
    // still finds the <think> block it expects for the 折叠块 UI.
    rawOutput = reasoningAccumulated
      ? `<think>\n${reasoningAccumulated}\n</think>\n\n${streamResult.content}`
      : streamResult.content;
  } else {
    rawOutput = await sendLLMRequest(apiConfig, preset, llmMessages, regexes, meta, requestOptions);
  }
