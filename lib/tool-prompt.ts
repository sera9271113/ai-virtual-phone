...
export function formatToolsForPrompt(tools: EnabledTool[]): string {
    ...
    const lines = [
        ...
        "2. 获取指令后，用 [执行动作:动作名(参数JSON)] 输出动作指令；不要编造动作。*如果上下文中已经获取过某个指令，则不要重复获取，需要时，直接执行前面获取过的指令即可*",
    ];

    if (hasSearch) {
        lines.push("");
        lines.push("**搜索策略**：当对话涉及互联网上可查证的话题（新闻、时事、真实人物/事件/产品/技术/地点/流行文化等），你应该主动搜索获取准确信息再回答，而不是凭记忆编造。讨论深入到新的细节时，应再次搜索更具体的关键词。宁可多搜一次也不要瞎编。");
    }
...
export function formatGroupToolsForPrompt(tools: EnabledTool[]): string {
    ...
    const lines = [
        ...
        '2. 获取指令后，用 ["角色名"执行动作:动作名(参数JSON)] 输出动作指令；必须用引号标注是哪个角色在执行动作，同一次回复中只能有一个角色执行动作。不要编造动作。*如果上下文中已经获取过某个指令，则不要重复获取，需要时，直接执行前面获取过的指令即可*',
    ];
