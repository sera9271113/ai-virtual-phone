...
import {
  buildCalendarScheduleMarker,
  clearGeneratedDayItems,
  clearGeneratedWeekItems,
  cloneDayPlanWithManualEdits,
  cloneWeekPlanWithManualEdits,
...
export async function generateWeeklyCalendarSchedule(
  ...
  try {
    ...
    const triggerInstruction = [
      ...
      `仅安排 ${formatCalendarHourLabel(CALENDAR_HOUR_START)} 到 ${formatCalendarHourLabel(CALENDAR_HOUR_END)} 之间的事项。`,
    ].join("\n");
...
export async function generateDailyCalendarSchedule(
  ...
  // 先清掉当天旧的 AI 生成条目（保留手动条目及其余日期），让随后的 marker 组装读不到旧结果——
  // 否则旧日程会进提示词被模型原样照抄，"重新生成"永远一字不差。失败时恢复。
  const removedGenerated = clearGeneratedDayItems(ownerType, ownerId, weekStart, targetDate);
  const restoreRemoved = () => restoreCalendarWeekItems(ownerType, ownerId, weekStart, removedGenerated);
  try {
    const resolved = await resolveCalendarAssemblerInput(ownerType, ownerId, weekStart);
    const triggerInstruction = [
      `请为${resolved.ownerName}生成 ${targetDate}（${getWeekdayLabel(targetDate)}）这一天的日程安排。`,
      "请参考已有日程，生成这一天的完整日程安排，不要生成其他日期的内容。",
      `仅安排 ${formatCalendarHourLabel(CALENDAR_HOUR_START)} 到 ${formatCalendarHourLabel(CALENDAR_HOUR_END)} 之间的事项。`,
    ].join("\n");
    ...
    const items = parseScheduleLines(rawText, weekStart).filter(item => item.date === targetDate);
    if (items.length === 0) {
      restoreRemoved();
      return { success: false, error: "日历生成结果为空，或格式无法解析。" };
    }

    cloneDayPlanWithManualEdits(ownerType, ownerId, weekStart, targetDate, items);
    return { success: true, items };
  } catch (error) {
    restoreRemoved();
    const err = error as ChatEngineError | Error;
