...
import { CALENDAR_MANAGEMENT_CAPABILITY_ID, DEVICE_BATTERY_CAPABILITY_ID, LOCAL_DATA_LIBRARY_CAPABILITY_ID, MEMORY_WRITE_CAPABILITY_ID, MUSIC_CONTROL_CAPABILITY_ID, NOTE_WALL_CAPABILITY_ID, SEND_FILE_CAPABILITY_ID, TIMED_WAKE_CAPABILITY_ID, TOOLBOX_MANAGEMENT_CAPABILITY_ID, getInternalCapability } from "./internal-capability-storage";
import { readDeviceBatteryStatus, formatDeviceBatteryStatusForTool } from "./device-battery";
...
async function proxyFetch(
    url: string,
    options: { method?: string; headers?: Record<string, string>; body?: unknown; signal?: AbortSignal; timeoutMs?: number; allowLocal?: boolean },
...
async function executeInternalTool(call: ToolCall, context?: ToolExecutionContext): Promise<ToolResult | null> {
    ...
    if (call.name === "查看设备电量") return executeDeviceBatteryTool();
...
async function executeDeviceBatteryTool(): Promise<ToolResult> {
    const capability = getInternalCapability(DEVICE_BATTERY_CAPABILITY_ID);
    if (!capability || !capability.enabled || capability.mode === "off") {
        return {
            name: "查看设备电量",
            success: false,
            error: "查看设备电量能力未启用",
            userNotice: "查看设备电量能力未启用",
        };
    }

    const status = await readDeviceBatteryStatus();
    return {
        name: "查看设备电量",
        success: true,
        data: formatDeviceBatteryStatusForTool(status),
        userNotice: status.supported ? `已查看设备电量（${status.level}%）` : "已查看设备电量（设备不支持）",
    };
}
...
type McpOAuthPendingState = {
    ...
    serverAllowLocalNetwork?: boolean;
    redirectUri: string;
    codeVerifier: string;
    clientId: string;
    clientSecret?: string;
...
async function mcpInitialize(server: McpServerConfig, signal?: AbortSignal): Promise<{ success: boolean; error?: string }> {
    ...
    }, authHeaders, false, useSse, signal, server.allowLocalNetwork);
    ...
    }, true, useSse, signal, server.allowLocalNetwork);
    ...
    try {
        ...
        if (!tokenEndpoint) {
            ...
            const probe = await mcpRequest(server.url, "initialize", {
                ...
                capabilities: {},
                clientInfo: MCP_CLIENT_INFO,
            }, undefined, false, undefined, signal, server.allowLocalNetwork);
            const resolved = await resolveMcpOAuthMetadata(server.url, probe.headers["www-authenticate"] || "", signal, server.allowLocalNetwork);
        ...
        const tokenRes = await proxyFetch(tokenEndpoint, {
            ...
            allowLocal: server.allowLocalNetwork,
        });
...
async function fetchJsonObject(url: string, signal?: AbortSignal, allowLocal?: boolean): Promise<unknown | null> {
    try {
        const res = await proxyFetch(url, { method: "GET", signal, timeoutMs: MCP_OAUTH_DISCOVERY_TIMEOUT_MS, allowLocal });
...
async function resolveAuthorizationServerMetadata(
    ...
    const fetched = await Promise.all(
        metadataUrls.map((metadataUrl) => fetchJsonObject(metadataUrl, signal, allowLocal).then((parsed) => ({ metadataUrl, parsed }))),
    );
...
async function resolveMcpOAuthMetadata(
    ...
    const fetched = await Promise.all(
        prmUrls.map((url) => fetchJsonObject(url, signal, allowLocal).then((parsed) => ({ url, parsed }))),
    );
    ...
    for (const { url: protectedResourceMetadataUrl, parsed } of fetched) {
        ...
        const resolvedList = await Promise.all(
            authorizationServers.map((authorizationServerUrl) =>
                resolveAuthorizationServerMetadata(authorizationServerUrl, signal, allowLocal).then((resolved) => ({ authorizationServerUrl, resolved })),
            ),
        );
...
export async function completePendingMcpOAuthCallback(): Promise<{ completed: boolean; success?: boolean; error?: string; serverName?: string }> {
    ...
    try {
        ...
        const tokenRes = await proxyFetch(pending.tokenEndpoint, {
            ...
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: new URLSearchParams(tokenBody).toString(),
            allowLocal: pending.serverAllowLocalNetwork,
        });
        ...
        const baseServer: McpServerConfig = idx >= 0 ? servers[idx] : {
            ...
            allowLocalNetwork: pending.serverAllowLocalNetwork,
...
async function executeMcpTool(server: McpServerConfig, toolName: string, args: Record<string, unknown>, signal?: AbortSignal): Promise<ToolResult> {
    try {
        ...
        const res = await mcpRequest(requestUrl, "tools/call", {
            name: toolName,
            arguments: args,
        }, getMcpSessionHeaders(server), false, useSse, signal, server.allowLocalNetwork);
...
export async function discoverMcpTools(serverUrl: string, server?: McpServerConfig): Promise<{ name: string; description: string; inputSchema: object }[]> {
    ...
    const res = await mcpRequest(serverUrl, "tools/list", {}, headers, false, useSse, undefined, server?.allowLocalNetwork);
...
export async function startMcpOAuth(server: McpServerConfig): Promise<{ success: boolean; error?: string }> {
    ...
    try {
        ...
        const probe = await mcpRequest(server.url, "initialize", {
            ...
            capabilities: {},
            clientInfo: MCP_CLIENT_INFO,
        }, undefined, false, undefined, undefined, server.allowLocalNetwork);
        ...
        try {
            resolvedMetadata = await resolveMcpOAuthMetadata(server.url, probe.headers["www-authenticate"] || "", undefined, server.allowLocalNetwork);
        } catch (err) {
            popup.close();
        ...
        writeStorageJson(MCP_OAUTH_PENDING_STORAGE_KEY, {
            ...
            serverAllowLocalNetwork: server.allowLocalNetwork,
            redirectUri,
            codeVerifier,
            clientId,
            clientSecret,
