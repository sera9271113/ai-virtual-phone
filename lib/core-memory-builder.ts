...
import { estimateMemorySignals } from "./memory-embedding";
...
export async function runCoreMemoryPipeline(
    ...
    // Reuses the same memory-summary API config; fails safe to {importance: 9, ...}.
    const signals = await estimateMemorySignals(summary, apiConfig, { minImportance: 7, fallbackImportance: 9 });
