            ...
            for (const event of parsed.events) {
                ...
                for (const dataLine of dataLines) {
                    ...
                    try {
                        ...
                        // Relay may send keepalive or non-JSON data. Skip it.
                        continue;
                    }
...
const DEFAULT_HTML_GENERATION_INSTRUCTION = `当用户的话里出现一个具体的、适合做成一张卡片/页面来呈现的"东西"时（例如：成绩单、菜单、票据、个人资料卡、小游戏、一段动态效果、一个可点击的互动界面等），
可以输出一个 \`\`\`html 代码块来代替纯文字回复。规则：
- 必须是完整自包含的 HTML 片段，内联 CSS 和 JS，不依赖外部资源
- 不要给 html/body/最外层容器设置 height: 100vh、100dvh、overflow: hidden、position: fixed 或 inset: 0；
  高度必须由内容自然撑起，允许纵向滚动
- 页面宽度需适配手机竖屏（建议 max-width: 380px 左右）
- 如果有可点击的选项按钮，用 <button data-action="选项文字"> 的形式，点击后会把文字发回对话
- 普通闲聊、没有具体可视化对象时，仍然用纯文字回复，不要为了用而用`;

export function buildChatHtmlInstruction(enabled: boolean | undefined, customPrompt?: string): string {
    if (enabled !== true) return "";
    if (customPrompt?.trim()) return customPrompt.trim();
    return DEFAULT_HTML_GENERATION_INSTRUCTION;
}
...
export async function buildChatPromptMessages(
    ...
    const now = new Date();
    const effectiveTimeZone = session.timeZone || character.timeZone || "Asia/Shanghai";
    const promptTimeContext = buildCharacterTimeContext(effectiveTimeZone, now);
    ...
    const offlineBilingualInstruction = !session.isGroup
        ...
        : "";
    const chatHtmlInstruction = !session.isGroup
        ? buildChatHtmlInstruction(session.htmlGenerationEnabled === true, session.htmlGenerationPrompt)
        : "";
    ...
    const llmMessages = assemblePromptPayload({
        ...
        htmlGenerationInstruction: chatHtmlInstruction,
