...
// 角色卡片色未指定 cardColor 时默认落到白色毛玻璃兜底（.ccf-card 的 FALLBACK_CARD_COLOR）。
// 不提供预设色板/随机上色——颜色完全交给用户在编辑页的色轮取色器自定义。
...
export function updateCharacter(id: string, updates: Partial<Character>): Character | null {
  ...
  const idx = chars.findIndex((char) => char.id === id);
...
export function createCharacter(
  ...
  return {
    ...
    tags: data.tags || [],
    cardColor: data.cardColor?.trim() || undefined,
    createdAt: now,
    updatedAt: now,
  };
}
...
export function parseCharacterFromJson(
  ...
  try {
    ...
    // Helper: validate cardColor — only accept hex (#fff/#ffffff) or hsl()/hsla() strings
    function validCardColor(v: unknown): string | undefined {
      if (typeof v !== "string" || !v.trim()) return undefined;
      const s = v.trim();
      if (/^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(s)) return s;
      if (/^hsla?\([^)]+\)$/.test(s)) return s;
      return undefined;
    }
    ...
    return {
      ...
      cardColor: validCardColor(src.cardColor ?? src.card_color),
    };
  } catch (e) {
