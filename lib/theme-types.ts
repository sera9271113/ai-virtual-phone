...
export type ThemeProfile = {
  ...
  // 锁屏背景深浅模式: "dark" = 深色背景用浅色毛玻璃, "light" = 浅色背景用深色毛玻璃
...
export const DEFAULT_THEME_PROFILE: ThemeProfile = {
  ...
  lockWallpaperY: 50,
  lockPassword: "",
  lockCustomText: "floatphone",
  lockThemeMode: "dark" as const,
  cssOverrides: {},
  globalCustomCSS: "",
...
export function normalizeThemeProfile(raw: unknown): ThemeProfile {
  ...
  base.lockWallpaperAssetId = typeof source.lockWallpaperAssetId === "string" ? source.lockWallpaperAssetId as string : null;
  base.lockWallpaperBlur = typeof source.lockWallpaperBlur === "number"
    ? Math.min(24, Math.max(0, source.lockWallpaperBlur as number)) : 0;
  base.lockWallpaperOpacity = typeof source.lockWallpaperOpacity === "number"
    ? Math.min(1, Math.max(0, Number((source.lockWallpaperOpacity as number).toFixed(3)))) : 0.9;
  base.lockWallpaperScale = typeof source.lockWallpaperScale === "number"
    ? Math.min(200, Math.max(10, source.lockWallpaperScale as number)) : 100;
  base.lockWallpaperX = typeof source.lockWallpaperX === "number"
    ? Math.min(100, Math.max(0, source.lockWallpaperX as number)) : 50;
  base.lockWallpaperY = typeof source.lockWallpaperY === "number"
    ? Math.min(100, Math.max(0, source.lockWallpaperY as number)) : 50;
  base.lockPassword = typeof source.lockPassword === "string" ? source.lockPassword as string : "";
  base.lockCustomText = typeof source.lockCustomText === "string" ? source.lockCustomText as string : "";
  base.lockThemeMode = source.lockThemeMode === "light" ? "light" : "dark";
