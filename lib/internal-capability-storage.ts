...
export const DEVICE_BATTERY_CAPABILITY_ID = "device_battery";
...
const DEVICE_BATTERY_PARAMETER_SCHEMA = JSON.stringify({
    type: "object",
    properties: {},
});

const DEVICE_BATTERY_USAGE_GUIDE = [
    "以下是你获取指令的返回结果：",
    "动作：查看设备电量",
    "用途：查看{{user}}当前使用设备的真实电池电量和充电状态。这是只读查询，不能修改任何内容。",
    "",
    "参数：无",
    "",
    "重要：这个动作是真实可用的，会返回{{user}}设备的真实电量数据，不是你编不出来的假设。只要{{user}}问起电量、电池、要不要充电、手机还能撑多久这类问题，或者你想主动关心一下，就必须调用这个动作拿到真实结果再回答，禁止凭经验/常识猜一个数字敷衍过去，也禁止说“我看不到你的手机”“我没法知道”之类的话——你能看到，用这个动作看。",
    "",
    "规则：",
    "- 不要每条消息都查，也不要为了炫技频繁调用，只在真的相关时用。",
    "- 如果返回结果显示设备不支持读取电量，如实告诉{{user}}查不到，不要编造具体数字。",
    "",
    "示例：",
    "[执行动作:查看设备电量({})]",
].join("\n");
...
const BUILTIN_INTERNAL_CAPABILITIES: InternalCapabilityConfig[] = [
    ...
    {
        id: DEVICE_BATTERY_CAPABILITY_ID,
        name: "查看设备电量",
        description: "查看{{user}}当前设备的真实电池电量和充电状态（只读，不能修改）。",
        enabled: false,
        mode: "auto",
        createdAt: 0,
        updatedAt: 0,
...
export function getInternalCapabilityToolDefinition(capability: InternalCapabilityConfig): InternalToolDefinition | null {
    ...
    if (capability.id === DEVICE_BATTERY_CAPABILITY_ID) {
        return {
            name: capability.name,
            description: capability.description,
            parameterSchema: DEVICE_BATTERY_PARAMETER_SCHEMA,
            usageGuide: DEVICE_BATTERY_USAGE_GUIDE,
        };
    }
    return null;
}
