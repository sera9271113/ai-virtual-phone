...
/**
 * 重新生成前清掉当天的 AI 生成条目（保留手动条目），逻辑同 clearGeneratedWeekItems，
 * 但只影响 targetDate 当天，其余日期的条目原样保留。
 */
export function clearGeneratedDayItems(
  ownerType: CalendarOwnerType,
  ownerId: string,
  weekStart: string,
  targetDate: string,
  ...
  const existing = loadCalendarWeekPlan(ownerType, ownerId, weekStart);
  const items = existing?.items ?? [];
  const removed = items.filter(item => item.date === targetDate && item.source !== "manual");
  if (removed.length === 0) return [];
  replaceCalendarWeekItems(
    ownerType,
    ownerId,
    weekStart,
    items.filter(item => !(item.date === targetDate && item.source !== "manual")),
  );
  return removed;
}
...
/**
 * 同 cloneWeekPlanWithManualEdits，但只替换 targetDate 当天的生成条目——
 * 其余日期（无论手动还是 AI 生成）原样保留，用于"只重新生成这一天"。
 */
export function cloneDayPlanWithManualEdits(
  ownerType: CalendarOwnerType,
  ownerId: string,
  weekStart: string,
  targetDate: string,
  ...
  const keptItems = existingItems.filter(item => item.date !== targetDate || item.source === "manual");
  const manualItemsForDay = existingItems.filter(item => item.date === targetDate && item.source === "manual");
  const nextItems = [...keptItems];
  for (const item of generatedItems.filter(entry => entry.source !== "manual")) {
    const collides = manualItemsForDay.find(
      entry =>
        entry.date === item.date &&
        entry.startTime === item.startTime &&
        entry.endTime === item.endTime &&
...
export function validateScheduleDraft(item: {
  ...
  if (!isCalendarTimeRangeAllowed(start, end)) {
    return `日程时间需在 ${formatCalendarHourLabel(CALENDAR_HOUR_START)} 到 ${formatCalendarHourLabel(CALENDAR_HOUR_END)} 之间`;
  }
