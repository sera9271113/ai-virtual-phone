...
const NINE_SLICE_CSS_SCHEMA = {
    type: "object",
    properties: {
        assetId: { type: "string", description: "素材 id；如果素材已有 publicUrl 会自动使用" },
...
const IMAGE_ASSET_USAGE_GUIDE = [
    ...
    "7. 图片气泡不允许自动猜九宫格参数。需要图片气泡时，先调用「校准九宫格」让用户在弹窗里手动拖切线；校准结果会返回 slice/display/padding 和 CSS。",
    "8. 「生成九宫格CSS」只接受已经校准好的完整参数；缺参数时会失败。不要用系统默认比例、不要自己瞎猜参数。",
    "9. 不要用 background-size: 100% 100% 或 background-size: cover 直接拉伸整张气泡图。",
].join("\n");
...
export const MASCOT_TOOL_PACKAGES: MascotToolPackage[] = [
    ...
    },
    {
        id: "image_pack",
        label: "图像处理套件",
        description: "生成、导入用户图、去底透明、裁切、压缩转换、列出读取和生成九宫格 CSS。适合制作聊天气泡、图标、背景纹理等主题素材。",
        subTools: [
            ...
            { name: "导入用户图片为素材", description: "把用户上传给小卷的图片导入 CSS 素材库，返回 assetId，后续可继续裁切、去底和写 CSS。", parameterSchema: IMPORT_USER_IMAGE_ASSET_SCHEMA },
